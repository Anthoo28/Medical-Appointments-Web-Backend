...
class AppointmentService {
  ...
  async createAppointment(appointmentData) {
    try {
      ...
      const availableTimes = isWeekday
        ? this.availableTimes.weekdays
        : this.availableTimes.weekends;

      const isTimeAvailable =
        appointmentData.time && availableTimes.includes(appointmentData.time);
      if (!isTimeAvailable) {
      ...
      const isAvailableForDoctor = await doctorService.isTimeAvailableForDoctor(
        doctor.dni,
      ...
      await doctorService.reserveTimeForDoctor(
        doctor.dni,
        appointmentData.date,
        appointmentData.time
      );

      const user = await this.userService.getUserByDni(appointmentData.user);
      const userEmail = user.email; // Extraer solo el correo electrónico del usuario
      if (!user) {
